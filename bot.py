import asyncio
import os
from telegram import Poll
from telegram.ext import ApplicationBuilder

# --- Bot token ---
BOT_TOKEN = os.environ.get("BOT_TOKEN")
if not BOT_TOKEN:
    raise ValueError("BOT_TOKEN environment variable is not set!")

# --- Chat / topic IDs ---
GROUP_ID = -1003893865263  # Your group ID
TOPIC_1_ID = 190            # Topic 1

# --- Poll options ---
OPTIONS = ["Eating", "Sleeping", "Working", "Exercising"]

# Interval between polls (seconds)
POLL_INTERVAL = 15 * 60  # 15 minutes


async def send_poll(app):
    """
    Sends a poll to Topic 1 and returns its poll ID.
    """
    message = await app.bot.send_poll(
        chat_id=GROUP_ID,
        message_thread_id=TOPIC_1_ID,
        question="What are you doing right now?",
        options=OPTIONS,
        is_anonymous=False,
        allows_multiple_answers=False
    )
    print(f"Poll sent! Poll ID: {message.poll.id}, Message ID: {message.message_id}")


async def poll_cycle(app):
    """
    Sends a poll every POLL_INTERVAL seconds.
    """
    while True:
        try:
            await send_poll(app)
        except Exception as e:
            print(f"Error sending poll: {e}")
        await asyncio.sleep(POLL_INTERVAL)


async def start_polling_loop(application):
    """
    Starts the poll cycle task once, immediately after the bot is ready.
    """
    if not hasattr(application, "_poll_cycle_started"):
        application._poll_cycle_started = True
        # Start poll immediately
        await send_poll(application)
        # Start background cycle
        asyncio.create_task(poll_cycle(application))


def main():
    # Build the bot application
    app = ApplicationBuilder().token(BOT_TOKEN).build()

    # Assign post_init to start polls
    app.post_init = start_polling_loop

    # Start the bot
    print("Bot starting...")
    app.run_polling()


if __name__ == "__main__":
    main()